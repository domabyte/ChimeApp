...
RCT_EXPORT_METHOD(switchCamera:(RCTPromiseResolveBlock)resolve rejecter:(RCTPromiseRejectBlock)reject) {
    ...
    // Attempt to switch the camera
    [meetingSession.audioVideo switchCamera];
    resolve(@"Camera switched successfully");
}

RCT_EXPORT_METHOD(switchMicrophoneToSpeaker:(RCTPromiseResolveBlock)resolve rejecter:(RCTPromiseRejectBlock)reject) {
    AVAudioSessionRouteDescription *route = [[AVAudioSession sharedInstance] currentRoute];
    BOOL isSpeakerEnabled = NO;
    ...
    NSError *error = nil;
    if (isSpeakerEnabled) {
        // Switch to the default audio route (usually the earpiece or connected headset)
        [[AVAudioSession sharedInstance] overrideOutputAudioPort:AVAudioSessionPortOverrideNone error:&error];
    } else {
        // Switch audio output to the built-in speaker
        [[AVAudioSession sharedInstance] overrideOutputAudioPort:AVAudioSessionPortOverrideSpeaker error:&error];
    }
    
    if (error) {
        reject(@"SWITCH_AUDIO_ROUTE_ERROR", @"Failed to switch audio route", error);
    } else {
        resolve(@"Audio route switched successfully");
    }
}
